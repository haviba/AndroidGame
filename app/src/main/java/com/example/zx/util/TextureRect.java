...
public class TextureRect {
    ...
    public TextureRect(int texId, float width, float height){
        ...
        ByteBuffer vbb = ByteBuffer.allocateDirect(vertices.length * 4); //因为一个整数4个字节
        vbb.order(ByteOrder.nativeOrder());       //设置字节顺序
        mVertexBuffer = vbb.asFloatBuffer();    //转化为int型缓冲
    ...
    public void drawSelf(GL10 gl) {
        ...
        gl.glTexCoordPointer(2, GL10.GL_FLOAT, 0, mTextureBuffer); //为画笔指定纹理坐标ST缓冲
