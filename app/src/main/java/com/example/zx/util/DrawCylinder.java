...
public class DrawCylinder {
    ...
    public DrawCylinder(float length, float circle_radius, int row, int col, float[][] yArray, int textureId){
        //圆柱是横向放置，中心点是原点。
        ...
        for(float circle_degree = 360.0f, i = 0;circle_degree > 0.0f; circle_degree -=rowSpan, i++){
            for(int j = 0; j < col; j++){
                ...
                val.add(x1);val.add(y1);val.add(z1);       //两个三角形，共六个顶点坐标
